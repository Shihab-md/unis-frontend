...
const Viewer = () => {
  ...
  useEffect(() => {
    ...
    const loadViewer = async () => {
      ...
      try {
        ...
        if (!currentItem?._id) {
          throw new Error("Demo - Tutorial file not found.");
        }
        setItem(currentItem);
        ...
        if (currentItem.fileKind === "VIDEO") {
          ...
          const { blob } = await demoTutorialApi.download({
            id: currentItem._id,
            fallbackFileName:
              currentItem.driveFileName || currentItem.originalFileName || "tutorial.pdf",
        ...
        const message = translateDemoTutorialMessage(
          ...
          error?.response?.data?.error ||
            error?.message ||
            "Unable to load Demo - Tutorial file."
        );
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      showSwalAlert(
        tr("Error!"),
        translateDemoTutorialMessage(tr, message),
        "error"
      );
      setDownloadProgress(0);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div
        className="mx-auto max-w-7xl p-3 md:p-5"
        dir={direction}
        style={{ fontFamily }}
      >
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-lg md:p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              ...
              <h1 className="break-words text-lg font-semibold text-slate-800 md:text-xl">
                {item?.title || tr("Demo - Tutorial")}
              </h1>
              {item?.description ? (
                <p className="mt-1 whitespace-pre-wrap break-words text-xs leading-5 text-slate-600 md:text-sm">
                  {item.description}
                </p>
              ) : null}
              {item ? (
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
                  <span className="inline-flex min-w-0 items-center gap-1.5">
                    {item.fileKind === "VIDEO" ? (
                      <FaVideo className="shrink-0 text-purple-700" />
                    ) : (
                      <FaFilePdf className="shrink-0 text-red-700" />
                    )}
            ...
            {item ? (
              <button
                ...
                className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md bg-blue-700 px-4 py-2 text-sm font-medium text-white shadow hover:bg-blue-800 disabled:cursor-not-allowed disabled:opacity-50"
              >
                <FaDownload />
                {downloading ? `${tr("Download")} ${downloadProgress}%` : tr("Download")}
              </button>
            ) : null}
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
          {loading ? (
            <div className="flex min-h-[55vh] flex-col items-center justify-center gap-3 p-6 text-sm text-slate-600">
              ...
              {previewProgress > 0 ? (
                <div className="w-full max-w-sm">
                  <div className="mb-1 text-center text-xs text-blue-700">{previewProgress}%</div>
                  <div className="h-1.5 overflow-hidden rounded-full bg-slate-200">
                    <div
                      className="h-full rounded-full bg-blue-700 transition-[width] duration-200"
            ...
            <div className="flex min-h-[45vh] items-center justify-center p-6 text-center text-sm text-red-700">
              {viewerError}
            </div>
          ) : item?.fileKind === "VIDEO" && streamUrl ? (
            <div className="flex min-h-[50vh] items-center justify-center bg-black p-0 md:p-4">
              <video
                key={streamUrl}
                className="max-h-[75vh] w-full max-w-6xl bg-black"
                src={streamUrl}
                controls
                playsInline
                preload="metadata"
                onError={() =>
                  setViewerError(tr("Unable to load Demo - Tutorial file."))
                }
              >
                {tr("Unable to load Demo - Tutorial file.")}
              </video>
            </div>
          ) : item?.fileKind === "PDF" && pdfUrl ? (
            <iframe
              title={item.title || tr("Demo - Tutorial")}
              src={`${pdfUrl}#toolbar=1&navpanes=0&scrollbar=1&view=FitH`}
              className="h-[72vh] min-h-[520px] w-full bg-white md:h-[78vh]"
            />
          ) : (
            <div className="flex min-h-[45vh] items-center justify-center p-6 text-center text-sm text-red-700">
              {tr("Unable to load Demo - Tutorial file.")}
